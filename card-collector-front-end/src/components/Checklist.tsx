...
function Checklist({
  ...
  setRefetchChecklistCollectionRefArr: React.Dispatch<
    React.SetStateAction<number[]>
  >;
}) {
  const [changedSinceLastSave, setChangedSinceLastSave] = useState<
    UpdatedChecklistItem[]
  >([]);
  ...
  function updateChangedSinceLastSave(item: UpdatedChecklistItem) {
    if (
      changedSinceLastSave.filter(
        (changedItem) => changedItem.cardId === item.cardId
      ).length < 1
    ) {
      // if this card hasnt been checked before
  ...
  function handleSave() {
    ...
    changedList.forEach((changedItem) => {
      if (oldList.includes(changedItem.cardId) && !changedItem.isChecked) {
        ...
        !oldList.includes(changedItem.cardId) &&
        changedItem.isChecked
      ) {
  ...
  async function updateUserChecklistApi(
    checklistId: string,
    updatedChecklist: number[],
    token: string
  ) {
    try {
  ...
  function renderChecklistItems(cardSet: UserChecklist, setName: string) {
    ...
    const productSets: CardSet[] =
      company[cardSet.season][cardSet.product].sets;
    ...
    for (let i = data.lowCardNum; i <= data.highCardnum; i++) {
      ...
      if (!isInCollection)
        data.missingCards.push((jsonSet.numPrefix || "") + i.toString());
  ...
  const percentage =
    (currentSavedList.length / renderedData.checklistItems.length) * 100;
  console.log(renderedData);

  function handleCopyMissingCards() {
    const lastIndex = renderedData.missingCards.length - 1;
    const string = renderedData.missingCards.reduce((acc, cur, i) => {
      return i === lastIndex ? acc + cur : acc + `${cur}, `;
    }, "");
  ...
  return (
    <article className="checklist">
      <button
        className="checklist__copy-btn"
        onClick={handleCopyMissingCards}
        title="Copy all missing card numbers to the clipboard"
      >
        <i className="fa-solid fa-clone"></i>
      </button>
      <button className="checklist__save-btn" onClick={handleSave} title="Save">
        <i className="fa-solid fa-floppy-disk"></i>
      </button>
      <button
        className="checklist__close-btn"
        onClick={handleClose}
        title="Close"
      >
      ...
      <section className="checklist__infobox">
        <h2 className="infobox__company">
          <span className="infobox__title">Company</span>
          {renderedData.companyName}
        </h2>
        <h2 className="infobox__season">
          <span className="infobox__title">Season</span>
          {checklist.season}
        </h2>
        <h2 className="infobox__product">
          <span className="infobox__title">Product</span>
          {renderedData.productName}
        </h2>
        <h2 className="infobox__set-name">
          <span className="infobox__title">Set name</span>
          {checklist.setName}
        </h2>
        <h2 className="infobox__set-type">
          <span className="infobox__title">Set type</span>
          {renderedData.setType}
        </h2>
        <h2 className="infobox__pack-odds">
          <span className="infobox__title">Pack odds</span>
          {renderedData.packOdds}
        </h2>
      ...
      <table className="checklist__table">
        ...
        <tbody>{renderedData.checklistItems}</tbody>
      </table>
      <button
        className="checklist__delete-btn"
        onClick={handleDelete}
        title="Delete - WARNING! The checklist will be permanently deleted."
      >
